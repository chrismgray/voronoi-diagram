...
(defn pt-within-seg-range? [pt seg]
  (let [pt-x (pt :x)
    ...
    (boolean
     (or
      (and (= 0 (first x-diffs)) (= 0 (first y-diffs)))
      (and (= 0 (second x-diffs)) (= 0 (second y-diffs)))
      (and
...
(defn find-bisector
  ...
  (let [s1-x (s1 :x)
        ...
        dummy-seg (if (infinite? bisector-slope)
                    (new-seg midpoint (pt/new-pt (midpoint :x) (+ 1 (midpoint :y))))
                    (new-seg midpoint (pt/new-pt (+ 1 (midpoint :x)) (+ (* bisector-slope (+ 1 (midpoint :x))) y-intercept))))
