...
(defn tree-update [do-update? how-to-update root]
  (let [walk (fn walk [node]
               (if (do-update? node)
                 ...
                 (if (seq? node)
                   (map walk node)
                   (if (map? node)
                     (zipmap (map walk (keys node)) (map walk (vals node)))
                     node))))]
    (walk root)))

(defn float-compare-op [op x]
  (->> x
       (tree-update rational? to-float)
       (apply op)))
